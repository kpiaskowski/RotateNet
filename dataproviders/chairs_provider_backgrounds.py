...
class ChairProvider:
    def __init__(self, path, background_path, batch_size, img_size, n_imgs=5, split_ratio=0.8, num_parallel_calls=12, seed=789432):
        self.batch_size = batch_size
        self.n_imgs = n_imgs
        self.img_size = img_size
        self.num_parallel_calls = num_parallel_calls
        self.class_paths = sorted([os.path.join(path, name) for name in os.listdir(path) if '.mat' not in name])
        self.background_paths = [os.path.join(background_path, name) for name in os.listdir(background_path)]
    ...
    def load_backrground_images(self, filename):
        image_string = tf.read_file(filename)
        image_decoded = tf.image.decode_jpeg(image_string, channels=3, try_recover_truncated=True)
        image_resized = tf.image.resize_images(image_decoded, [self.img_size, self.img_size]) / 255
    ...
    def background_dataset(self):
        dataset = tf.data.Dataset.from_tensor_slices((tf.constant(self.background_paths)))
        dataset = dataset.shuffle(buffer_size=5000)
        dataset = dataset.map(self.load_backrground_images, self.num_parallel_calls)
