...
note = 'small inp, small out with batchnorm'
...
with tf.Session() as sess:
    train_writer = tf.summary.FileWriter('summaries/' + model_name + '_train', graph=sess.graph)
    ...
    for i in range(iters):
        ...
        _, cost, summ = sess.run([train_op, mse_loss, loss_merged], feed_dict={base_pl: base, target_pl: target, handle: t_handle, is_training: True})
        ...
        if t_s % ckpt == 0:
            ...
            img_summ = sess.run(img_merged, feed_dict={base_pl: base, target_pl: target, handle: t_handle, is_training: False})
