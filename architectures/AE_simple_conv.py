...
class AE_only_conv:
    ...
    def encoder(self, imgs, activation, is_training):
        with tf.variable_scope('encoder', reuse=tf.AUTO_REUSE, initializer=xavier_initializer(), regularizer=l2_regularizer(0.01)):
            e_conv = tf.layers.conv2d(imgs, 48, 3, padding='same', activation= activation, name='econv1')
            e_conv = tf.layers.max_pooling2d(e_conv, 2, 2)

            e_conv = tf.layers.conv2d(e_conv, 92, 3, padding='same', activation= activation, name='econv2')
            e_conv = tf.layers.max_pooling2d(e_conv, 2, 2)

            e_conv = tf.layers.conv2d(e_conv, 256, 3, padding='same', activation= activation, name='econv3')
            e_conv = tf.layers.max_pooling2d(e_conv, 2, 2)

            e_conv = tf.layers.conv2d(e_conv, 256, 3, padding='same', activation= activation)
            e_conv = tf.layers.max_pooling2d(e_conv, 2, 2)
    ...
    def decoder(self, merged_lv, activation, is_training):
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE, initializer=xavier_initializer(), regularizer=l2_regularizer(0.01)):
            ...
            d_conv = tf.layers.conv2d(d_conv, 256, 3, padding='same', activation= activation)
            d_conv = tf.image.resize_images(d_conv, (32, 32))

            d_conv = tf.layers.conv2d(d_conv, 92, 3, padding='same', activation= activation)
            d_conv = tf.image.resize_images(d_conv, (64, 64))

            d_conv = tf.layers.conv2d(d_conv, 48, 3, padding='same', activation= activation)
